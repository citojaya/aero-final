...
void readInput(char *infile, int *np, real *dens, real *ymod, 
			real *pois, real *sfc, real *rec, real *dmpn, real *rf, real *cyldia,
			 real *dt, int *nW, int *updateDPM, real *maxVel, int *rotZoneID){
	// input file reading
	char filename[20];
	strcpy(filename, infile);
	strcat(filename ,".in"); 
...
void readWallMesh(char *infile, int mode){
	...
	// if(mode == 1){
	// 	readFaces(noOfRotorFaces, wFile, wFaceRot);
	// }
	...
	if(mode == 2){
		readFaces(noOfStatorFaces, wFile, wFace);
	}
