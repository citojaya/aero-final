...
DEFINE_DPM_INJECTION_INIT(solid_paritcles, I)
{
  ...
  //Setup DEM scaling 
  setReduceUnits();
  writeLogNum("wallmesh.log","minRotEdgeLength ", minRotEdgeLength);
  writeLogNum("wallmesh.log","maxRotEdgeLength ", maxRotEdgeLength);
  writeLogNum("wallmesh.log","minStatEdgeLength ", minEdgeLength);
  writeLogNum("wallmesh.log","maxStatEdgeLength ", maxEdgeLength);
  writeLogLine("wallmesh.log","setReduceUnits()");
  //Assign wall faces to bounding box
  //assignFaceToBDBox(wFaceRot, noOfRotorFaces, 1);
  assignFaceToBDBox(wFace, noOfStatorFaces, 2);
  // assignFace(1, wFace, statZoneID);
  writeLogLine("wallmesh.log","assignFaceToBDBox\n");
  assignRotFace(1, wFaceRot, rotZoneID);
  writeLogLine("wallmesh.log","assignRotFace\n");
  refineBdBoxFace(wFace);
  writeLogLine("wallmesh.log","refineBdBoxFace\n");
  writeLogNum("wallmesh.log","max faces in a BD box (stator) ",maxFaces);
  writeLogNum("wallmesh.log","max faces in a BD box (rotor)",maxRotFaces);
  ...
  loop(I2,Ilist)
  {
    ...
    loop(p,I2->p_init)
    {
      int iIndex = floor((demPart[p->part_id].pos[0]-xmin)/domainDx);
      int jIndex = floor((demPart[p->part_id].pos[1]-ymin)/domainDy);
      int kIndex = floor((demPart[p->part_id].pos[2]-zmin)/domainDz);
  ...
  //writeLogNum("wallmesh.log","NUM OF WALL FACES ",noOfFaces)
  //writeLog3Num("wallmesh.log","wall grid divisions ",wBXDiv,wBYDiv,wBZDiv);
  //writeLog3Num("wallmesh.log","wall grid size ",wBoxDx/lengthFactor,wBoxDy/lengthFactor,wBoxDz/lengthFactor);
...
DEFINE_PROFILE(bed_por,thread, np)
{
  ...
  begin_c_loop(c,thread)
  {
    ...
    if(initialized)
    {
      ...
      cfdcell[cI].porosity = fmax(0.25, 1.0 - cfdcell[cI].solidVol/(C_VOLUME(c,thread)));
...
DEFINE_EXECUTE_AT_END(execute_at_end)
{
  ...
  //Update rotor wall face
  assignRotFace(0, wFaceRot, rotZoneID);
