...
real getOverlap(real *parPos, real dia, real *n1, real *n2, real *n3, real *uVec){ 
    ...
    //writeLogNum("wallmesh.log","mag of v2 ",vecMag(v2)/lengthFactor);
    vecSub(v1,v2,ppDash);
    // uVec[0] = ppDash[0];
    // uVec[1] = ppDash[1];
    // uVec[2] = ppDash[2];
    ...
    // unitVec(uVec,uVec);
    //writeLogNum("wallmesh.log","par pos ",parPos[0]/lengthFactor);       
...
void findContactFromMesh(Particle *p, real *dVec){
    //getNearFace(p->part_id);
    //if(nFace > 0){
        //writeLogNum("wallmesh.log","near face", nFace);
        real *uVec = allocateDoubleArray(DIM);

        real gap = vecMag(dVec) - 0.5*demPart[p->part_id].dia;
        unitVec(dVec,uVec);

        if(gap < 100e-9*lengthFactor){pWallVWForce(p->part_id, gap, uVec);}
        if(gap < 0) //If contact exists calculate contact force
        {
            //writeLogNum("wallmesh.log","GAP",gap/lengthFactor);
            if(contactExist(p->part_id, nFace) == 0){
                    //charge(p->part_id, -1);
                addWallContact(p->part_id, nFace);
                demPart[p->part_id].noOfWallColl += 1;
            ...
            //delete all contact except nFace
            deleteAllContacts(p->part_id,nFace);
            surfaceContactForce(p->part_id, -gap, uVec);
        }
        // else if(gap > 0){
        //     deleteWallContact(p->part_id,nFace);
        // }
        free(uVec);
    //}        
}
